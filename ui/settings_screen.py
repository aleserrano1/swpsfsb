...
class SettingsScreen(ctk.CTkFrame):
    ...
    def _build_ui(self):
        label(self, "Settings", bold=True, size=22, fg="#ffffff").pack(anchor="w", padx=32, pady=(24, 4))
        ...
        button(scroll, "Save Settings", self._save, width=160,
               fg_color="#7e67f5", hover_color="#6952d4").pack(anchor="w", pady=(20, 8))

    def _build_company_section(self, parent, company: str, name: str):
        frame = ctk.CTkFrame(parent, corner_radius=16, fg_color="#0d1826")
        frame.pack(fill="x", pady=8)

        label(frame, name, bold=True, size=14, fg="#ffffff").pack(anchor="w", padx=20, pady=(16, 6))

        for field_key, placeholder in [
            (f"{company}_president", "President Name"),
        ]:
            section_label(frame, placeholder.upper()).pack(anchor="w", padx=20, pady=(4, 0))
            e = entry(frame, placeholder=placeholder, width=380)
            e.pack(anchor="w", padx=20, pady=(0, 4))
            e.insert(0, get(field_key))
            self._fields[field_key] = e

        phone_key = f"{company}_phone"
        section_label(frame, "PHONE NUMBER").pack(anchor="w", padx=20, pady=(6, 0))
        e = phone_entry(frame, placeholder="Phone Number", width=380)
        e.pack(anchor="w", padx=20, pady=(0, 4))
        e.insert(0, sanitize_phone(get(phone_key)))
        self._fields[phone_key] = e

        section_label(frame, "COMPANY ADDRESS").pack(anchor="w", padx=20, pady=(10, 2))
        ...
        city_row.pack(anchor="w", padx=20, pady=(0, 4))
        for field_key, placeholder, width in [
        ...
        section_label(frame, "COMPANY LOGO").pack(anchor="w", padx=20, pady=(10, 2))
        logo_row = ctk.CTkFrame(frame, fg_color="transparent")
        logo_row.pack(anchor="w", padx=20, pady=(0, 4))
        ...
        button(logo_row, "Browse", lambda e=logo_entry: self._browse_logo(e), width=80,
               fg_color="#1e2d45", hover_color="#263452").pack(side="left")

        ctk.CTkFrame(frame, height=16, fg_color="transparent").pack()
    ...
    def _build_directory_section(self, parent):
        frame = ctk.CTkFrame(parent, corner_radius=16, fg_color="#0d1826")
        frame.pack(fill="x", pady=8)
        label(frame, "Output Directory", bold=True, size=14, fg="#ffffff").pack(anchor="w", padx=20, pady=(16, 6))
        section_label(frame, "BASE FOLDER FOR PROJECT FILES").pack(anchor="w", padx=20, pady=(4, 0))

        row = ctk.CTkFrame(frame, fg_color="transparent")
        row.pack(anchor="w", padx=20, pady=(4, 16))
        ...
        button(row, "Browse", self._browse_dir, width=80,
               fg_color="#1e2d45", hover_color="#263452").pack(side="left")
    ...
    def _build_pin_section(self, parent):
        frame = ctk.CTkFrame(parent, corner_radius=16, fg_color="#0d1826")
        frame.pack(fill="x", pady=8)
        label(frame, "Security", bold=True, size=14, fg="#ffffff").pack(anchor="w", padx=20, pady=(16, 6))
        button(frame, "Change PIN", self._change_pin, width=140,
               fg_color="#1e2d45", hover_color="#263452").pack(anchor="w", padx=20, pady=(4, 16))
